...
> 日期: 2026-03-05
> 状态: Draft
> 关联: `tutorial-system-design.md`, `fl.c` → `fl_hello()`

## 1. 背景

当前教学引导共 11 步（welcome → complete），覆盖了各面板功能介绍，但缺少一个**端到端的注入实操环节**。用户走完教学后仍不清楚"注入到底是什么效果"。

固件侧已新增 `fl_hello` 函数（`__attribute__((noinline))`，非 static），通过 `fl -c hello` 调用，输出：

```
Hello from original fl_hello!
...
|------|-----|------|---------|-----------|
| 10 | `hello_search` | Symbols | 搜索 `fl_hello` 符号并双击创建补丁标签页 | Editor 中存在 `fl_hello` 补丁标签页 |
| 11 | `hello_inject` | Editor | 辑补丁代码并点击 Inject 注入 | 对应 slot 状态变为 active |
| 12 | `hello_verify` | — (居中) | 运行 hello 命令观察注入效果，然后 Unpatch 再运行观察原始效果 | 无 gate（纯观察步骤） |

原 `complete` 步骤顺延为步骤 13。

## 3. 步骤详细设计

### 3.1 步骤 10: `hello_search` — 搜索符号

**目标**: 让用户在 Symbols 面板搜索 `fl_hello`，双击符号创建补丁标签页。
...
```js
{
  ...
  gate: () => {
    // 检查 Editor 标签页中是否存在 fl_hello 的补丁标签
...
**渲染内容**:
- 说明：在搜索框输入 `fl_hello`，找到该符号后双击打开补丁编辑器
- 3 个 feature-item：
  1. 🔍 输入搜索关键词 — 在 Symbol Search 输入框中输入 `fl_hello`
...
fl_println("Hello from INJECTED fl_hello!");
fl_response(true, "HELLO injected");
```
...
### 3.3 步骤 12: `hello_verify` — 验证注入效果

**目标**: 用户通过 Quick Commands 运行 `fl -c hello` 观察注入后的输出，然后 Unpatch 再运行一次观察恢复原始输出。
...
**阶段 A — 验证注入生效**:
1. 在 Quick Commands 面板输入 `fl -c hello` 并发送
2. 在 Serial 日志中观察输出 — 应该看到你修改后的消息
3. 确认 `[FLOK] HELLO injected`（或你自定义的响应）

**阶段 B — 取消注入并对比**:
1. 在 Device Info 面板找到已注入的 slot，点击 ✕ 按钮取消注入（或点击 Clear All）
2. 再次发送 `fl -c hello`
3. 观察输出恢复为原始消息：`Hello from original fl_hello!`
...
hello_inject_example

# hello_verify 步骤
hello_verify_title
hello_verify_desc
hello_verify_phase_a
hello_verify_phase_a_desc
hello_verify_phase_b
hello_verify_phase_b_desc
hello_verify_hint

# Gate 消息
gate_hello_search
gate_hello_search_ok
gate_hello_inject
gate_hello_inject_ok
```
...
| `hello_search_desc` | 在符号面板中搜索 fl_hello，双击创建补丁 |
| `hello_inject_title` | 编辑并注入补丁 |
| `hello_inject_desc` | 修改补丁代码中的输出消息，然后注入到设备 |
| `hello_verify_title` | 验证注入效果 |
| `hello_verify_desc` | 运行 hello 命令对比注入前后的输出差异 |
| `gate_hello_search` | 请在符号面板搜索 fl_hello 并双击创建补丁 |
| `gate_hello_search_ok` | 补丁标签页已创建！ |
| `gate_hello_inject` | 请修改补丁代码后点击 Inject 按钮注入 |
...
- [ ] 在 `stepRenderers` 中添加 `hello_search()`、`hello_inject()`、`hello_verify()` 渲染函数
- [ ] `hello_inject` 渲染器中包含代码示例（用 `<pre><code>` 展示建议修改）
- [ ] `hello_verify` 渲染器中用两个分组（Phase A / Phase B）展示操作步骤
...
- [ ] 验证 `hello_search` gate：双击符号后标签页出现 → gate 通过
- [ ] 验证 `hello_inject` gate：注入成功后 slot active → gate 通过
- [ ] 验证 `hello_verify`：无 gate，Next 按钮始终可用
...
| 风险 | 影响 | 缓解措施 |
|------|------|---------|
| 用户跳过 config 步骤 | 符号搜索失败 | 在 `hello_search` 渲染器中检测 ELF 路径是否配置，未配置时显示提示 |
| 设备未连接 | 注入失败 | `hello_inject` gate 依赖 slot active，注入失败时 gate 不通过，用户可 Skip |
| `fl_hello` 不在 ELF 中 | 搜索无结果 | 固件侧已确保 `noinline` + 非 static，编译后必有符号；文档中注明需使用包含该函数的固件版本 |
