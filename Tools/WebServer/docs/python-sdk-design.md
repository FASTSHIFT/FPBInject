...
5. **薄门面，不重复造轮子**：`Client` 内部委托现有的 `ServerProxy` / `FPBInject` / `discover_sync`，只做统一封装与稳定化，不改设备协议。
...
|------|----------|-----------------|------|
| **ELF 分析** | `analyze(elf, func)` | `analyze` | 离线 |
| | `disasm(elf, func)` | `disasm` | 离线 |
| | `decompile(elf, func)` | `decompile` | 离线（需 Ghidra） |
| | `signature(elf, func)` | `signature` | 离线 |
| | `search(elf, pattern)` | `search` | 离线 |
| | `get_symbols(elf, filter=None, limit=0)` | `get-symbols` | 离线 |
| | `compile(source, *, elf=None, base_addr=..., compile_commands=None)` | `compile` | 离线 |
| **设备信息** | `info()` | `info` | 代理 |
| | `test_serial(...)` | `test-serial` | 代理 |
| **注入** | `inject(target_func, source, *, elf=None, patch_mode="trampoline", comp=-1)` | `inject` | 代理 |
| | `unpatch(comp=0, all=False)` | `unpatch` | 代理 |
| **内存** | `mem_read(addr, length, fmt="hex")` | `mem-read` | 代理 |
| | `mem_write(addr, hexdata)` | `mem-write` | 代理 |
| | `mem_dump(addr, length, out_path)` | `mem-dump` | 代理 |
| **串口** | `serial_send(data)` | `serial-send` | 代理 |
| | `serial_read(since=0)` | `serial-read` | 代理 |
| **文件** | `file_list(path="/")` | `file-list` | 代理 |
| | `file_stat(path)` | `file-stat` | 代理 |
| | `file_download(remote, local)` | `file-download` | 代理 |
| | `file_upload(local, remote)` | `file-upload` | 代理 |
| | `file_remove(path)` | `file-remove` | 代理 |
| | `file_mkdir(path)` | `file-mkdir` | 代理 |
| | `file_rename(old, new)` | `file-rename` | 代理 |
| **连接** | `connect(port, baudrate=...)` | `connect` | 代理 |
...
> 覆盖率：CLI 的每个子命令都有对应 SDK 方法。SDK 是 CLI 的超集入口（多了 `ensure_server`、`status`、上下文管理等便利能力）。
