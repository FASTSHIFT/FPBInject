...
class TestSerialThroughputFragmentIntegration(unittest.TestCase):
    ...
    def test_download_runs_before_full_upload_sweep(self):
        """Read test must run before the heavy upload sweep, so download
        reliability isn't measured against an upload-overloaded link."""
        events = []
        self.protocol._phase_fragment_probe = lambda timeout=2.0: {"needed": False}

        def fake_echo(size, timeout=2.0):
            events.append(("echo", size))
            return {"passed": True, "response_time_ms": 1.0, "cmd_len": size * 2}

        def fake_echoback(size, timeout=3.0):
            events.append(("echoback", size))
            return {"passed": size <= 256, "size": size, "response_time_ms": 1.0}

        self.protocol._probe_echo = fake_echo
        self.protocol._probe_echoback = fake_echoback

        results = self.protocol.test_serial_throughput(
            start_size=16, max_size=64, timeout=0.1, trials=1
        )
        self.assertTrue(results["success"])

        kinds = [e[0] for e in events]
        first_echoback = kinds.index("echoback")
        # The full upload sweep steps past start_size (16 -> 22 -> ...); those
        # larger echo probes must all come after the download (echoback) test.
        larger_echoes = [
            i for i, e in enumerate(events) if e[0] == "echo" and e[1] > 16
        ]
        self.assertTrue(larger_echoes)
        self.assertTrue(all(i > first_echoback for i in larger_echoes))

    def test_short_upload_check_gates_download(self):
        """If the minimum write size is unreliable, bail out before the read
        test and report the connection failure."""
        self.protocol._phase_fragment_probe = lambda timeout=2.0: {"needed": False}
        self.protocol._probe_echo = lambda size, timeout=2.0: {
            "passed": False,
        ...
        echoback_called = []
        self.protocol._probe_echoback = lambda size, timeout=3.0: (
            echoback_called.append(size) or {"passed": True, "size": size}
        )

        results = self.protocol.test_serial_throughput(
            start_size=16, max_size=64, timeout=0.1, trials=2
        )

        self.assertFalse(results["success"])
        self.assertEqual(results["failed_size"], 16)
        self.assertIn("upload_short", results["phases"])
        # Download must not have been attempted.
        self.assertEqual(echoback_called, [])

    def test_start_size_not_sampled_twice(self):
        """The short check result seeds the sweep, so start_size is measured
        once, not again at the head of the full upload sweep."""
        self.protocol._phase_fragment_probe = lambda timeout=2.0: {"needed": False}
        echo_sizes = []

        def fake_echo(size, timeout=2.0):
            echo_sizes.append(size)
            # Only the minimum size passes, so the sweep stops right after it.
            return {
                "passed": size <= 16,
                "response_time_ms": 1.0,
                "cmd_len": size * 2,
                "error": None if size <= 16 else "timeout",
            }
        ...
        self.protocol.test_serial_throughput(
            start_size=16, max_size=64, timeout=0.1, trials=3
        )

        # start_size (16) sampled 3 times for the short check; the sweep reuses
        # that result rather than sampling 16 again. The sweep then steps to 22
        # (x1.4) which fails, so 22 is sampled once (3 trials).
        self.assertEqual(echo_sizes.count(16), 3)
