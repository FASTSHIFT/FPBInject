      ...
      if (rawResult.logs && rawResult.logs.length > 0) {
        rawResult.logs.forEach((entry) => {
          if (rawTerm) {
            ...
            if (entry.dir === 'TX') {
              rawTerm.writeln(
                `\x1b[33m[${timestamp}] TX:\x1b[0m ${entry.data}`,
              );
            } else {
              ...
              lines.forEach((line, idx) => {
                if (line.trim() || idx === 0) {
                  if (idx === 0) {
                    rawTerm.writeln(
                      `\x1b[36m[${timestamp}] RX:\x1b[0m ${line}`,
                    );
                  } else {
...
async function connect() {
  ...
  const baudrate =
    parseInt(document.getElementById('baudrate').value) || 115200;
...
async function refreshStatus() {
  ...
  if (result.success) {
    ...
    if (result.watcher_enabled !== undefined) {
      ...
      if (
        result.watcher_enabled &&
        result.watch_dirs &&
        result.watch_dirs.length > 0
      ) {
...
async function searchSymbols() {
  ...
  const result = await api(
    '/symbols?q=' + encodeURIComponent(query) + '&limit=50',
  );
  if (result.success) {
    renderSymbolList(result.symbols);
  }
...
async function generatePatch() {
  ...
  const result = await api('/patch/generate', 'POST', {
...
async function savePatchToFile() {
  ...
  const result = await api('/patch/source', 'POST', {
    content,
    save_to_file: true,
  });
  if (result.success) {
...
async function performInject() {
  ...
  if (result.success) {
    ...
    if (term) {
      ...
      term.writeln(
        '\x1b[32m║         \x1b[1m✓ 注入成功\x1b[22m                       ║\x1b[0m',
      );
      term.writeln(
        '\x1b[32m╠══════════════════════════════════════════╣\x1b[0m',
      );
      ...
      term.writeln(
        `\x1b[32m║\x1b[0m  目标地址: \x1b[33m${result.target_addr}\x1b[0m`,
      );
      term.writeln(
        `\x1b[32m║\x1b[0m  注入函数: \x1b[36m${result.inject_func}\x1b[0m`,
      );
      term.writeln(
        `\x1b[32m║\x1b[0m  注入地址: \x1b[33m${result.inject_addr}\x1b[0m`,
      );
      term.writeln(
        '\x1b[32m╠══════════════════════════════════════════╣\x1b[0m',
      );
      term.writeln(
        `\x1b[32m║\x1b[0m  代码大小: \x1b[35m${result.code_size} bytes\x1b[0m`,
      );
      term.writeln(
        `\x1b[32m║\x1b[0m  编译耗时: \x1b[35m${result.compile_time}s\x1b[0m`,
      );
      term.writeln(
        `\x1b[32m║\x1b[0m  上传耗时: \x1b[35m${result.upload_time}s\x1b[0m`,
      );
      term.writeln(
        `\x1b[32m║\x1b[0m  总计耗时: \x1b[35m${result.total_time}s\x1b[0m`,
      );
      term.writeln(
        `\x1b[32m║\x1b[0m  Patch模式: \x1b[34m${result.patch_mode}${nuttxModeActive ? ' (NuttX)' : ''}\x1b[0m`,
      );
      term.writeln(
        '\x1b[32m╚══════════════════════════════════════════╝\x1b[0m',
      );
...
async function navigateTo(path) {
  const filterParam = browserFilter
    ? `&filter=${encodeURIComponent(browserFilter)}`
    : '';
  const result = await api(
    `/browse?path=${encodeURIComponent(path)}${filterParam}`,
  );
...
function selectBrowserItem() {
  const path =
    browserSelectedPath || document.getElementById('browserPath').value;
...
async function detectModifiedFunctions() {
  ...
  const result = await api('/patch/detect_changes', 'POST', {
    file_path: filePath,
  });
  ...
  if (result.success) {
    ...
    if (lastDetectedModifiedFuncs.length === 0) {
      listEl.innerHTML =
        '<span class="hint-text">未检测到修改的函数 (与 git HEAD 相同)</span>';
    } else {
      listEl.innerHTML = lastDetectedModifiedFuncs
        .map((func) => `<span class="func-tag modified">${func}</span>`)
        .join(' ');
    }

    if (term) {
      term.writeln(
        `\x1b[36m[Detect]\x1b[0m Found ${lastDetectedModifiedFuncs.length} modified functions: ${lastDetectedModifiedFuncs.join(', ') || 'none'}`,
      );
    }
  } else {
...
async function autoGeneratePatch() {
  ...
  const result = await api('/patch/auto_generate', 'POST', {
    file_path: filePath,
  });
  ...
  if (result.success) {
    ...
    if (term) {
      term.writeln(
        `\x1b[32m[Patch Generated]\x1b[0m ${lastDetectedModifiedFuncs.length} functions: ${result.injected_functions?.join(', ') || 'none'}`,
      );
    }
  } else {
...
function copyAutoPatchContent() {
  ...
  if (
    content &&
    content !== '点击 "生成 Patch" 查看结果...' &&
    !content.startsWith('生成')
  ) {
    navigator.clipboard.writeText(content).then(() => {
      if (term) {
        term.writeln(
          '\x1b[32m[Copied]\x1b[0m Patch content copied to clipboard',
        );
      }
    });
  }
...
function useAutoPatchAsSource() {
  ...
  if (term) {
    term.writeln(
      '\x1b[32m[Applied]\x1b[0m Patch content applied to source editor',
    );
  }
}
...
async function refreshAutoInjectStatusWithLogging() {
  ...
  // Update message
  document.getElementById('autoInjectMessage').textContent =
    message || '等待文件变化...';

  // Update modified functions display
  const funcsEl = document.getElementById('autoInjectFuncs');
  if (modifiedFuncs.length > 0) {
    funcsEl.innerHTML = modifiedFuncs
      .map((func) => `<span class="func-tag modified">${func}</span>`)
      .join(' ');
  } else {
    funcsEl.innerHTML = '';
